...
void fight(int& weapon, int& pet, double & money)
{
	std::mt19937 gen;
	gen.seed(time(0));
...
void Shop(double &money, int &weapon, int &upgrade1, int &upgrade2, double &cost, int &pet , struct pet2 PIKACHU)
{
	...
	if (Text_Shop == "PIKA")
	{
		PIKACHU.pikachu(weapon);
		Shop(money, weapon, upgrade1, upgrade2, cost, pet, (pet2(PIKACHU)));
	}
...
void start_game(double& money, int& weapon, int& pet, double& cost, int& upgrade1, int& upgrade2, struct boss tom, struct  boss ivan,struct pet2 PIKACHU)
{
	const auto n = 1;
	std::string m;
	while (n > -10)
	...
	start_game(money, weapon, pet, cost, upgrade1, upgrade2, (boss(tom)), (boss(ivan)),pet2(PIKACHU));
	return;
}
